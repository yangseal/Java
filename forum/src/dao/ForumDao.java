...
public class ForumDao {
	...
	//修改一个用户
	public void updateUser(String userName, int flag) throws SQLException {
		session = HibernateSessionFactory.getSession();
		transaction = session.beginTransaction();
		/*session.update(user);
		transaction.commit();
		...
		if (flag==0) {
			...
			String procedure = "{call UpdateUser_add(?) }";
			CallableStatement cstmt = con.prepareCall(procedure);
			cstmt.setString(1, userName);
			cstmt.executeUpdate();
		}
	...
	public List<Post> personalPost(User user,int pageNumber,int pageSize){
		...
		session = HibernateSessionFactory.getSession();
		String hql = "from Post as post_view where post_view.user.userId = ? order by post_view.postTime desc";
	...
	public int personalPostAmount(User user){
		...
		session = HibernateSessionFactory.getSession();
		String hql = "select count(*) from Post as post_view where post_view.user.userId = ?";
		query = session.createQuery(hql);
		query.setInteger(0, user.getUserId());
	...
	//模糊查询获取所有帖子总数
	public int postAmount(String  name){
		int postAmount = 0;
		session = HibernateSessionFactory.getSession();
		String hql = "select  count(*) from  User user   where  user.userName like :t";

		query = session.createQuery(hql);
		query.setString("t", "%" +name +"%");
		long  l = (Long)query.uniqueResult();
		postAmount = (int)l;
		session.close();
		return postAmount;
	...
	//模糊查询所有帖子
	public List<Post> allPost(int pageNumber ,int pageSize, String name){
		session = HibernateSessionFactory.getSession();
		List<Post> allPost = new ArrayList<Post>();
		String hql = "from  User user   where  user.userName like :t";
		query = session.createQuery(hql);
		query.setString("t", "%" +name.trim() +"%");
		query.setFirstResult((pageNumber - 1)*pageSize);
		query.setMaxResults(pageSize);
		allPost = query.list();
		session.close();
	...
	{
		List <Integer> personNum = new ArrayList<>();
		session = HibernateSessionFactory.getSession();

		String hql = "select Pcount  from  Number";
		query = session.createQuery(hql);

		personNum = query.list();
		session.close();
		return personNum;
	}
}
